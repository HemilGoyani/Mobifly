...
def create_module(module_name, db):
    ...
    if exist_module:
        raise HTTPException(
            ...
            detail=f"Module {module_name} is exist",
        )
    ...
    for role in roles:
        if role.name == "Admin":
            permission(role.id, create_module.id, AccessName.READ_WRITE.value, db)
        else:
            permission(role.id, create_module.id, AccessName.READ.value, db)
...
def delete_module(module_id, db):
    ...
    if not first_module:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, detail=f" Module id {module_id} not found"
        )
    permission_check = db.query(Permission).filter(Permission.module_id == module_id)
    delete_data(permission_check, db)
...
def get_module_permission(module_id, db):
    ...
    if not check_module:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, detail=f"Module_id {module_id} not found"
        )

    # get module permission
    get_module_permission = (
        db.query(Permission).filter(Permission.module_id == module_id).all()
    )
