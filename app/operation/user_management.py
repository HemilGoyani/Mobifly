...
async def forgot_paswords_email_sent(user_id, email, db):
    ...
    if not userobj:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="email and id not found"
        )
...
def change_password(id, data, db):
    ...
    if user.password == hash_password_old.hexdigest():
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="oldpassword is not valid"
        )
...
def check_reset_password_token(reset_password_token, db):
    ...
    get_token = (
        db.query(Email_token)
        .filter(
            Email_token.reset_code == reset_password_token,
            Email_token.status == True,
            Email_token.expired_in >= now - datetime.timedelta(minutes=10),
        )
        .first()
    )
    return get_token
...
def update_user_role_permission(user_id, role_id, data, db):
    ...
    raise HTTPException(
        status.HTTP_404_NOT_FOUND, detail="role_id is not valid for this user"
    )
